...
public class CyclicBarrier {
    ...
    private int dowait(boolean timed, long nanos)
        ...
        lock.lock();  //加锁
        try {
            final Generation g = generation;    //当前屏障（代）
            // 屏障被破坏，抛出异常
            if (g.broken)
                throw new BrokenBarrierException();
            // 线程被中断
            if (Thread.interrupted()) {
                breakBarrier(); //损坏当前屏障，并且唤醒所有的线程
            ...
            if (index == 0) {  // tripped 所有线程都已经进入屏障了，该线程是最后一个进入屏障的
                ...
                try {
                    final Runnable command = barrierCommand;    // 需要执行的任务
                    if (command != null)
                        command.run();  // 执行任务
            ...
            for (;;) {
                try {
                    ...
                    if (g == generation && ! g.broken) {
                        ...
                        Thread.currentThread().interrupt(); // 当前屏障已经结束，或者屏障已经被破坏，则重设中断标识位
                    }
                }

                if (g.broken)   // 如果屏障被破坏，则抛出异常
                    throw new BrokenBarrierException();

                if (g != generation) // 屏障结束
                    return index;

                if (timed && nanos <= 0L) { // 设置了等待时间，并且时间超时了，破坏屏障，抛出异常
                    breakBarrier();
