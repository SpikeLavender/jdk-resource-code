...
public final class Unsafe {
   ...
   /**
    ...
    private native long allocateMemory0(long bytes);    //在本地内存分配一块指定大小的新内存，内存的内容未初始化;它们通常被当做垃圾回收。
    private native long reallocateMemory0(long address, long bytes);    //重新分配给定内存地址的本地内存
    private native void freeMemory0(long address);  //释放给定地址的内存
    private native void setMemory0(Object o, long offset, long bytes, byte value); //将给定内存块中的所有字节设置为固定值（通常是0）
    @HotSpotIntrinsicCandidate
    private native void copyMemory0(Object srcBase, long srcOffset, Object destBase, long destOffset, long bytes); //复制一块内存，double-register模型
    private native void copySwapMemory0(Object srcBase, long srcOffset, Object destBase, long destOffset, long bytes, long elemSize);
    private native long objectFieldOffset0(Field f);    //获取给定对象的偏移地址
    private native long objectFieldOffset1(Class<?> c, String name);    //获取给定对象的偏移地址
    private native long staticFieldOffset0(Field f);    //获取给定对象的偏移地址
    private native Object staticFieldBase0(Field f);
    private native boolean shouldBeInitialized0(Class<?> c);
    private native void ensureClassInitialized0(Class<?> c);
    private native int arrayBaseOffset0(Class<?> arrayClass);   //获取给定数组的第一个元素的偏移地址
    private native int arrayIndexScale0(Class<?> arrayClass);   //获取给定数组的元素增量地址，也就是说每个元素的占位数
    private native int addressSize0();
    private native Class<?> defineAnonymousClass0(Class<?> hostClass, byte[] data, Object[] cpPatches); //定义匿名内部类
